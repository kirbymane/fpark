...
async function upsertEmail(
  ...
  try {
    ...
    const toAddresses = email.to
      .map((addr: { address: string }) => addressMap.get(addr.address))
      .filter(Boolean);
    ...
    const thread = await db.thread.upsert({
      ...
      update: {
        ...
        participantIds: [
          ...new Set([
            fromAddress.id,
            ...toAddresses.map((a: any) => a!.id),
            ...ccAddresses.map((a: any) => a!.id),
            ...bccAddresses.map((a: any) => a!.id),
          ]),
        ],
      },
      create: {
    ...
    await db.email.upsert({
      ...
      update: {
        ...
        to: { set: toAddresses.map((a: any) => ({ id: a!.id })) },
        cc: { set: ccAddresses.map((a: any) => ({ id: a!.id })) },
        bcc: { set: bccAddresses.map((a: any) => ({ id: a!.id })) },
        replyTo: { set: replyToAddresses.map((a: any) => ({ id: a!.id })) },
      ...
      create: {
        ...
        meetingMessageMethod: email.meetingMessageMethod,
        fromId: fromAddress.id,
        to: { connect: toAddresses.map((a: any) => ({ id: a!.id })) },
        cc: { connect: ccAddresses.map((a: any) => ({ id: a!.id })) },
        bcc: { connect: bccAddresses.map((a: any) => ({ id: a!.id })) },
        replyTo: { connect: replyToAddresses.map((a: any) => ({ id: a!.id })) },
        hasAttachments: email.hasAttachments,
        body: email.body,
        bodySnippet: email.bodySnippet,
        inReplyTo: email.inReplyTo,
